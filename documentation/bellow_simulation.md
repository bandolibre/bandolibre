...
P  = clamp(P, -BELLOW_INTENSITY_MAX, +BELLOW_INTENSITY_MAX)
```
...
from centre. With the defaults (`dir_dead=1024`, `dir_hyst=512`), direction commits
when `|P| > 768` and releases when `|P| < 256`.

Expression (CC#11, paired with CC#43 as the 14-bit LSB) is emitted from the same
effective intensity through the shared 1-euro-filter + hysteresis + rate-limit
pipeline used by the pedals: a speed-adaptive low-pass (`bellow_cc_1e_mincutoff`/
`bellow_cc_1e_beta`) does the primary noise suppression, a small residual backlash
(`bellow_cchyst`, 0 disables it) mops up anything left, and the rate limit caps
...
returns to rest (intensity = 0), CC=0 is sent immediately and the pipeline state
is reset so filtering/backlash restart cleanly on the next gesture.
...
| `dir_dead`                | 1024    | Neutral deadzone in pressure units (0–`BELLOW_INTENSITY_MAX`). Entry threshold is `dir_dead/2 + dir_hyst/2` from zero. |
| `dir_hyst`                | 512     | Margin given back before returning to neutral, in pressure units. Larger holds the direction (and a note) longer after the blade returns. |

The expression-CC behaviour reuses the existing `bellow_cc_1e_mincutoff`,
`bellow_cc_1e_beta`, `bellow_cchyst` and `bellow_cc_period_ms`; the FN1
sensitivity scale still applies, since the model is fed the already-scaled
reading.
...
2. Set `dir_dead` to match the plain-mode deadzone in pressure units:
   ```
   dir_dead ≈ (bellow_dead/2 + bellow_hyst/2) / push_span × BELLOW_INTENSITY_MAX
   ```
   With illustrative values (`bellow_dead=40`, `bellow_hyst=20`, push span ≈ 360
   ADC units, the same figures this worked example has always used --
   independent of the live `bellow_dead`/`bellow_hyst` property defaults, which
   are calibrated per unit): `dir_dead ≈ 30/360 × 16384 ≈ 1365`.
